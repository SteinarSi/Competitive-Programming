...
main = do
    ...
    putStrLn $ runST $ do
        ...
        bfs seen table accepting ss [s] <&> bool "non-empty" "empty"

bfs :: STUArray s Int Bool -> UArray (Int,Char) Int -> UArray Int Bool -> C.ByteString -> [Int] -> ST s Bool
bfs seen table accepting sigma [] = pure True
bfs seen table accepting sigma xs 
    | any (accepting!) xs = pure False
